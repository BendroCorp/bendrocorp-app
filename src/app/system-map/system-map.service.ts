...
import { StarSystem, Planet, Moon, Settlement, SystemMapTypes, SystemObject, SystemLocation, SystemImage, MissionGiver, JumpPoint, GravityWell, SystemMapSearchItem } from '../models/system-map-models';
...
export class SystemMapService {
  ...
  fetchViewSelection(): number {
    let smv = localStorage.getItem('system-map-view-selection');
    if (smv) {
      return parseInt(smv);
    } else {
  ...
  recentSelectedListItems(): SystemMapSearchItem[] {
    let recents = JSON.parse(localStorage.getItem('system-map-view-recent-items')) as SystemMapSearchItem[];
    return recents;
  }
